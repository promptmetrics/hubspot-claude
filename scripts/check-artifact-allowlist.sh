...
# output-styles/ SKILL.md README.md LICENSE pyproject.toml .gitignore
# scripts/check-artifact-allowlist.sh (this script is committed so CI can run it)
set -u

allow_regex='^(src/|tests/|bin/hubspot$|hooks/|\.claude-plugin/|\.github/|scripts/check-artifact-allowlist\.sh$|output-styles/|SKILL\.md$|README\.md$|LICENSE$|pyproject\.toml$|\.gitignore$)'
