...
def test_cli_loop_flag_runs_run_loop(tmp_path, monkeypatch):
    _setup_portal(tmp_path, monkeypatch)

    with patch("hubspot_agent.cli.run_loop", return_value="loop result") as mock_run_loop:
...
def test_cli_multi_agent_defaults_to_loop(tmp_path, monkeypatch):
    _setup_portal(tmp_path, monkeypatch)
...
def test_cli_single_agent_uses_flat_path(tmp_path, monkeypatch):
    _setup_portal(tmp_path, monkeypatch)
