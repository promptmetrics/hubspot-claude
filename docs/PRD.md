...
- **Killed/deferred subsystems** (`08-roadmap.md:246-268`): DAG planner, sandbox preview, RBAC, plugin architecture v2, hooks, replay tooling, anomaly detection, reflection engine, batch coalescing, pattern-approval mode, parallel write execution.
...
- [ ] **Phase 2 — partial-capture AUTO (deferred, review MINOR).** A bulk update where some preview GETs failed has partial `original_values` → still `is_undoable` → AUTO-applies, but a later undo restores only the captured subset. Consider requiring full capture (captured == records targeted) for AUTO eligibility, else downgrade to CONFIRM.
- [ ] **Phase 2 — AUTO ceiling risk appetite.** Default `auto_apply_max_records=100` means a fat-fingered reversible payload mutates up to 100 records with no pause (undo + audit bound it). Revisit the default or add a first-run notice — explicit product call.
...
| R11 Bounded Autonomy tiers | Done (uncommitted) | `policy.py` (`classify_write` + `load_approval_policy`); wired in `safety.py`/`handlers.py`/`cli.py`; `never_auto_tools` gate for side-effectful workflow writes. Full suite green on Python 3.12 (1143 passed, 1 skipped). Adversarial review passed after 2 fixes (union-merge safety lists; `ExecuteError`→recoverable). 9 pre-Phase-2 tests migrated to the new contract (coverage preserved, not weakened). Uncommitted; no version bump yet. |
| R12 Loop cost governance | Not started | Phase 3; today's `$0.50` ceiling is inert (`loop_controller.py:130-140`). |
| R13 Back-pressure / retry-backoff | Not started | Phase 3; today's `_drive_loop` fails-and-stops per step (`orchestrator.py:906-918`). |
