...
- **Unattended mutation stays a non-goal.** Scheduled tasks (R15) run reads unattended but only ever *stage* writes as pending previews; execution always requires a human `approve` (drift-checked by compare-and-set). No auto-apply tier runs without a person, in a loop or on a schedule.
...
- **R15.** Scheduled / recurring tasks — register a recurring job (concrete tool-path plan + cron) that an external timer replays via `hubspot schedule run-due`; the run executes reads inline and **stages every write as a pending preview** (deferred approval), never mutating unattended — acceptance: a due schedule stages its writes with zero outbound mutation; a staged write approved later re-verifies via compare-and-set (drift → skip); a schedule is skipped while its prior batch is unreviewed and un-freezes once the batch is resolved; a queued batch past `schedule_queue_ttl_days` expires; a free-text (non-concrete) step is refused unattended; the interactive loop is unaffected; full suite green.
...
- **2026-07-22** — **Write-detection alignment (v0.2.14; safety fix to R2/R5).** The interactive durable loop's approval gate (`is_write_step`) used a narrower verb set than the executor's intent parser (`_parse_agent_intent`), so a free-text step with a synonym verb (`remove`/`clear`/`set`/`modify`/…) classified as a read at the gate but a write at execution — inline-executing without a pause. Fixed at the root by a shared `classify_intent_type` (single source of truth in `sequential_dispatch.py`) that both use; the gate change is additive (nothing that gated before stops gating). Found by the R15 pre-ship review, which had already guarded scheduled runs against the same divergence.
- **2026-07-22** — **Scheduled / recurring tasks shipped (v0.2.13; new R15).** Reconciled the three-way conflict — every write needs approval, the durable loop plans/verifies with Claude in-session, and a cron-fired run has neither Claude nor a human — with four choices: (1) **queue-for-review + compare-and-set** — a run stages every write as a pending preview capturing the pre-image; nothing mutates until a human `approve`, drift-checked however long the gap; (2) **external timer** (`launchd`/`cron`) invokes `hubspot schedule run-due` — no always-on daemon; (3) **concrete plan captured at creation** — Claude builds the verbatim-tool plan once at `schedule add`; cron replays it deterministically with no run-time LLM (free-text/non-concrete steps are refused, in the CLI and defensively in `_drive_loop`, precisely because the free-text agent branch could mutate unattended); (4) **skip-while-pending** — a schedule doesn't re-run while its prior batch is unreviewed, and un-freezes as soon as the batch is resolved (the overlap gate re-derives "pending" from whether the previews still exist on disk, not a stored status). Staged previews are exempt from the 24h reaper and live to `schedule_queue_ttl_days` (default 7). Adversarial review before ship caught + fixed the branch-3 unattended-mutation hole, the freeze-until-TTL bug, and the reaper/TTL mismatch. Spec: `docs/superpowers/specs/2026-07-22-scheduling-design.md`.
...
| R15 Scheduled / recurring tasks | Done (v0.2.13) | `hubspot schedule {add,list,remove,run-due,install-timer}`; external timer replays a stored concrete plan; `_drive_loop` stage-and-continue mode (reads inline, every write staged, zero unattended mutation); free-text steps refused; overlap gate re-derives pending from disk + `schedule_queue_ttl_days` expiry; deferred approval keeps compare-and-set. New `cron.py`, `schedule_store.py`; `loop_state.py`/`orchestrator.py`/`cli.py`/`policy.py`/`persistence.py`. |
...
| 4 | Scheduled / recurring tasks (queue-for-review) | R15 | Complete (v0.2.13) |
