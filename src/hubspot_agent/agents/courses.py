...
_DOMAIN = (
    "You manage courses in HubSpot. "
    "You retrieve, search, create, update, and delete course records."
)

_DEFAULT_OBJECT_TYPE = "courses"
...
_STOP_WORDS = {
    ...
    "change", "edit", "modify", "delete", "remove", "course", "courses",
}
...
def get_courses_agent_prompt(portal_config: PortalConfig | None = None) -> AgentPrompt:
    tools = [t for name in _TOOL_NAMES if (t := get_tool(name)) is not None]
    return build_agent_prompt(
        agent_name="Courses Agent",
...
@register_preview("courses")
async def _build_courses_preview(
    agent_name: str,
    intent: TaskIntent,
    client,
    portal_id: str,
...
@register_execute("courses")
async def _execute_courses(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
...
@register_reconcile("courses")
async def _reconcile_courses(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
