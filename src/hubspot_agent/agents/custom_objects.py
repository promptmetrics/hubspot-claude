...
_STOP_WORDS = {
    ...
    "change", "edit", "modify", "delete", "remove",
}
...
@register_preview("custom_objects")
async def _build_custom_objects_preview(
    agent_name: str,
    intent: TaskIntent,
    client,
    portal_id: str,
    ...
    if not intent.target_object:
        return PreviewResult(
            preview={"message": f"{intent.intent_type} operation on custom_objects"},
    ...
    if intent.intent_type in ("search", "update", "delete"):
        ...
        try:
            result = await invoke_tool(
                ...
                query={"query": search_term, "limit": 10, "properties": ["name"]},
                client=client,
            )
    ...
    if intent.intent_type == "create":
        return PreviewResult(
            preview={"message": f"Will create a new {intent.target_object} custom object record"},
            impact_count=1,
            risk_level=intent.risk_level,
...
@register_execute("custom_objects")
async def _execute_custom_objects(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
    ...
    if not intent.target_object:
        return {"status": "success", "message": f"Executed custom_objects for: {request_text}"}
...
@register_reconcile("custom_objects")
async def _reconcile_custom_objects(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
