...
_DOMAIN = (
    "You manage HubSpot commerce taxes. "
    "You retrieve, search, create, update, and delete tax records. "
    "Taxes can be linked to fees and discounts."
)

_DEFAULT_OBJECT_TYPE = "taxes"
...
_STOP_WORDS = {
    ...
    "change", "edit", "modify", "delete", "remove", "tax", "taxes",
}
...
def get_taxes_agent_prompt(portal_config: PortalConfig | None = None) -> AgentPrompt:
    tools = [t for name in _TOOL_NAMES if (t := get_tool(name)) is not None]
    return build_agent_prompt(
        agent_name="Taxes Agent",
...
@register_preview("taxes")
async def _build_taxes_preview(
    agent_name: str,
    intent: TaskIntent,
    client,
    portal_id: str,
...
@register_execute("taxes")
async def _execute_taxes(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
...
@register_reconcile("taxes")
async def _reconcile_taxes(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
