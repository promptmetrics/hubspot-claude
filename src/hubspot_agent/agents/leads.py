...
_DOMAIN = (
    "You manage leads in HubSpot. "
    "You retrieve, search, create, update, and delete lead records. "
    "Lead pipelines are available on Pro/Enterprise tiers."
)

_DEFAULT_OBJECT_TYPE = "leads"
...
_STOP_WORDS = {
    ...
    "change", "edit", "modify", "delete", "remove", "lead", "leads",
}
...
def get_leads_agent_prompt(portal_config: PortalConfig | None = None) -> AgentPrompt:
    tools = [t for name in _TOOL_NAMES if (t := get_tool(name)) is not None]
    return build_agent_prompt(
        agent_name="Leads Agent",
...
@register_preview("leads")
async def _build_leads_preview(
    agent_name: str,
    intent: TaskIntent,
    client,
    portal_id: str,
...
@register_reconcile("leads")
async def _reconcile_leads(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
