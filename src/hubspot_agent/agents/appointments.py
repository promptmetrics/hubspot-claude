...
_DOMAIN = (
    "You manage appointments in HubSpot. "
    "You retrieve, search, create, update, and delete appointment records."
)

_DEFAULT_OBJECT_TYPE = "appointments"
...
_STOP_WORDS = {
    ...
    "change", "edit", "modify", "delete", "remove", "appointment", "appointments",
}
...
def get_appointments_agent_prompt(portal_config: PortalConfig | None = None) -> AgentPrompt:
    tools = [t for name in _TOOL_NAMES if (t := get_tool(name)) is not None]
    return build_agent_prompt(
        agent_name="Appointments Agent",
...
@register_preview("appointments")
async def _build_appointments_preview(
    agent_name: str,
    intent: TaskIntent,
    client,
    portal_id: str,
...
@register_execute("appointments")
async def _execute_appointments(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
...
@register_reconcile("appointments")
async def _reconcile_appointments(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
