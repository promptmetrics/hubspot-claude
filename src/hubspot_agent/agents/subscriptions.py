...
_DOMAIN = (
    "You manage HubSpot commerce subscriptions. "
    "You retrieve, search, create, update, and delete subscription records. "
    "Subscriptions handle recurring billing cycles."
)

_DEFAULT_OBJECT_TYPE = "subscriptions"
...
_STOP_WORDS = {
    ...
    "change", "edit", "modify", "delete", "remove", "subscription", "subscriptions",
}
...
def get_subscriptions_agent_prompt(portal_config: PortalConfig | None = None) -> AgentPrompt:
    tools = [t for name in _TOOL_NAMES if (t := get_tool(name)) is not None]
    return build_agent_prompt(
        agent_name="Subscriptions Agent",
...
@register_preview("subscriptions")
async def _build_subscriptions_preview(
    agent_name: str,
    intent: TaskIntent,
    client,
    portal_id: str,
...
@register_execute("subscriptions")
async def _execute_subscriptions(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
...
@register_reconcile("subscriptions")
