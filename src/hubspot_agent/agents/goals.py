...
_DOMAIN = (
    "You manage HubSpot sales goals. "
    "You retrieve, search, create, update, and delete goal target records. "
    "Goals track sales team performance against targets."
)

_DEFAULT_OBJECT_TYPE = "goal_targets"
...
_STOP_WORDS = {
    ...
    "change", "edit", "modify", "delete", "remove", "goal", "goals",
...
def get_goals_agent_prompt(portal_config: PortalConfig | None = None) -> AgentPrompt:
    tools = [t for name in _TOOL_NAMES if (t := get_tool(name)) is not None]
    return build_agent_prompt(
        agent_name="Goals Agent",
...
@register_preview("goals")
async def _build_goals_preview(
    agent_name: str,
    intent: TaskIntent,
    client,
    portal_id: str,
