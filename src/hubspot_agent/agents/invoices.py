...
_DOMAIN = (
    "You manage HubSpot commerce invoices. "
    "You retrieve, search, create, update, and delete invoice records for accounts receivable and billing."
)

_DEFAULT_OBJECT_TYPE = "invoices"
...
_STOP_WORDS = {
    ...
    "change", "edit", "modify", "delete", "remove", "invoice", "invoices",
}
...
def get_invoices_agent_prompt(portal_config: PortalConfig | None = None) -> AgentPrompt:
    tools = [t for name in _TOOL_NAMES if (t := get_tool(name)) is not None]
    return build_agent_prompt(
        agent_name="Invoices Agent",
...
@register_preview("invoices")
async def _build_invoices_preview(
    agent_name: str,
    intent: TaskIntent,
    client,
    portal_id: str,
...
@register_execute("invoices")
async def _execute_invoices(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
