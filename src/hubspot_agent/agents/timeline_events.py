...
_DOMAIN = (
    "You manage custom timeline events in HubSpot. "
    "You retrieve, search, create, update, and delete timeline event records, "
    "templates, and tokens."
)

_DEFAULT_OBJECT_TYPE = "timeline_events"
...
_STOP_WORDS = {
    ...
    "change", "edit", "modify", "delete", "remove", "timeline", "event", "events",
}
...
def get_timeline_events_agent_prompt(portal_config: PortalConfig | None = None) -> AgentPrompt:
    tools = [t for name in _TOOL_NAMES if (t := get_tool(name)) is not None]
    return build_agent_prompt(
        agent_name="Timeline Events Agent",
...
@register_preview("timeline_events")
async def _build_timeline_events_preview(
    agent_name: str,
    intent: TaskIntent,
    client,
    portal_id: str,
...
@register_execute("timeline_events")
async def _execute_timeline_events(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
...
@register_reconcile("timeline_events")
async def _reconcile_timeline_events(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
