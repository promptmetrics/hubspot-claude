...
_DOMAIN = (
    "You manage HubSpot commerce discounts. "
    "You retrieve, search, create, update, and delete discount records. "
    "Discounts can be linked to quotes and invoices."
)

_DEFAULT_OBJECT_TYPE = "discount"
...
_STOP_WORDS = {
    ...
    "change", "edit", "modify", "delete", "remove", "discount", "discounts",
}
...
def get_discounts_agent_prompt(portal_config: PortalConfig | None = None) -> AgentPrompt:
    tools = [t for name in _TOOL_NAMES if (t := get_tool(name)) is not None]
    return build_agent_prompt(
        agent_name="Discounts Agent",
...
@register_preview("discounts")
async def _build_discounts_preview(
    agent_name: str,
    intent: TaskIntent,
    client,
    portal_id: str,
...
@register_execute("discounts")
async def _execute_discounts(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
...
@register_reconcile("discounts")
async def _reconcile_discounts(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
