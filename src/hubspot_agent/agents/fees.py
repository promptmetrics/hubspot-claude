...
_DOMAIN = (
    "You manage HubSpot commerce fees. "
    "You retrieve, search, create, update, and delete fee records. "
    "Fees can be linked to quotes and invoices."
)

_DEFAULT_OBJECT_TYPE = "fees"
...
_STOP_WORDS = {
    ...
    "change", "edit", "modify", "delete", "remove", "fee", "fees",
}
...
def get_fees_agent_prompt(portal_config: PortalConfig | None = None) -> AgentPrompt:
    tools = [t for name in _TOOL_NAMES if (t := get_tool(name)) is not None]
    return build_agent_prompt(
        agent_name="Fees Agent",
...
@register_preview("fees")
async def _build_fees_preview(
    agent_name: str,
    intent: TaskIntent,
    client,
    portal_id: str,
...
@register_execute("fees")
async def _execute_fees(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
...
@register_reconcile("fees")
async def _reconcile_fees(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
