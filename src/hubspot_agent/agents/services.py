...
_DOMAIN = (
    "You manage services in HubSpot. "
    "You retrieve, search, create, update, and delete service records."
)

_DEFAULT_OBJECT_TYPE = "services"
...
_STOP_WORDS = {
    ...
    "change", "edit", "modify", "delete", "remove", "service", "services",
}
...
def get_services_agent_prompt(portal_config: PortalConfig | None = None) -> AgentPrompt:
    tools = [t for name in _TOOL_NAMES if (t := get_tool(name)) is not None]
    return build_agent_prompt(
        agent_name="Services Agent",
...
@register_preview("services")
async def _build_services_preview(
    agent_name: str,
    intent: TaskIntent,
    client,
    portal_id: str,
...
@register_reconcile("services")
async def _reconcile_services(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
