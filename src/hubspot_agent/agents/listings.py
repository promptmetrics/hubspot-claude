...
_DOMAIN = (
    "You manage listings in HubSpot. "
    "You retrieve, search, create, update, and delete listing records."
)

_DEFAULT_OBJECT_TYPE = "listings"
...
_STOP_WORDS = {
    ...
    "change", "edit", "modify", "delete", "remove", "listing", "listings",
}
...
def get_listings_agent_prompt(portal_config: PortalConfig | None = None) -> AgentPrompt:
    tools = [t for name in _TOOL_NAMES if (t := get_tool(name)) is not None]
    return build_agent_prompt(
        agent_name="Listings Agent",
...
@register_preview("listings")
async def _build_listings_preview(
    agent_name: str,
    intent: TaskIntent,
    client,
    portal_id: str,
...
@register_execute("listings")
async def _execute_listings(
    agent_name: str,
    intent: TaskIntent,
    request_text: str,
    client,
